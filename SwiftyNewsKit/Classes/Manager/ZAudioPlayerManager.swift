...
public enum ZEnumAudioPlayerState {
...
open class ZAudioPlayerManager: NSObject, AVAudioPlayerDelegate {
    ...
    open private(set) var state: ZEnumAudioPlayerState = .stopped
    ...
    private func startProgressTimer() {
        progressTimer?.invalidate()
        progressTimer = nil
        progressTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(ZAudioPlayerManager.didFireProgressTimer(_:)), userInfo: nil, repeats: true)
    }
