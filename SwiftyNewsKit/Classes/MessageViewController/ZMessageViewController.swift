...
open class ZMessageViewController: MessagesViewController {
    ...
    public var loginUser: ZModelMessageUser {
        let loginUser = ZSettingKit.shared.userLogin ?? ZModelUserLocal.init()
    ...
    /// 接受者用户对象
    public var modelUser: ZModelUserLocal? {
        didSet {
    ...
    lazy var audioPlayManager = ZAudioPlayerManager(messageCollectionView: self.messagesCollectionView)
    ...
    open override func viewWillAppear(_ animated: Bool) {
        ...
        ZAudioRecordManager.shared.delegate = self
    ...
    open override func viewWillDisappear(_ animated: Bool) {
        ...
        ZAudioRecordManager.shared.delegate = nil
    ...
    /// 发送语音
    final func startSendAudio(_ uploadData: Data, _ recordTime: Float, _ filepath: URL) {
        ...
        model.message = L10n.messageisaVoice
    ...
    final func startSendImages(_ images: [UIImage]) {
        ...
        /// 添加一条消息
        let messageid = kRandomId
        let message = ZModelMessageType.init(image: image, imageUrl: imageUrl, sender: senderUser, receive: receiveUser, messageId: messageid, sentDate: sendTime)
        self.insertMessage(message)
        self.messageInputBar.invalidatePlugins()
        /// 保存消息对象
        let model = ZModelMessage.init()
        model.message_userid = receiveId
        model.message_id = messageid
        model.message = L10n.messageisaPicture
        model.message_type = .image
        model.message_time = sendTime.timeIntervalSince1970
        model.message_read_state = true
        model.message_file_id = imageUrl.lastPathComponent
    ...
    /// 发送文本
    final func startSendMessage(_ text: String) {
        // 文本内容不能为空
...
extension ZMessageViewController {
    ...
    @objc private func btnAudioPressStartEvent(_ sender: ZMessageButton) {
        // 获取录音权限
        ZAudioRecordManager.shared.checkPermissionAndSetupRecord()
        BFLog.debug("start record audio")
        self.viewRecordAudio.startRecord()
        ZAudioRecordManager.shared.startRecord()
